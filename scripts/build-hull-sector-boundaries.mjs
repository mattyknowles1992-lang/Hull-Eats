...
const parseSectorFile = (fileName) => {
  ...
  if (!HULL_OUTWARDS.includes(outward) || !/^[0-9]$/.test(sector)) {
    return null;
  }
...
const indexPath = join(root, "packages", "types", "src", "hull-sector-boundaries-index.generated.ts");
const index = {};
for (const feature of features) {
  ...
  index[outward].push(feature.properties.sector);
}
for (const outward of Object.keys(index)) {
  index[outward].sort();
}
...
const indexLines = [
  ...
  ...Object.keys(index)
    ...
    .map((outward) => `  ${JSON.stringify(outward)}: ${JSON.stringify(index[outward])},`),
  "};",
  "",
];
writeFileSync(indexPath, `${indexLines.join("\n")}\n`, "utf8");
console.log(`Wrote sector index to ${indexPath}`);
